...
# Response variable: Total Relative Eveness (Jost 2010) [ln*hill2]/[ln*hill1]
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
diversity <- diversity %>% 
  mutate(rle12 = log(tot.hill.2)/log(tot.hill.1))
...
#   geom_quasirandom()
# (plot2 <- ggplot(data_all, aes(x = rle12)) +
#   geom_histogram(binwidth = 1))
# plot3 <- ggplot(data_all, aes(x = rle12)) +
#   geom_density()
# plot4 <- ggplot(data_all, aes(x = log(rle12))) +
#   geom_density()
...
library(rstatix)
data_all %>% 
  select(id.site, rle12) %>% 
  identify_outliers(rle12)
# extreme outliers in N_DAM and S_JAN
data_all %>% 
...
# filter outliers
data_all <- data_all %>% 
  filter(!id.site %in% c("N_DAM", "S_JAN") )
...
ggplot(data_all, aes(x = rest.meth, y = rle12)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
  labs(title = "Restoration method")
ggplot(data_all, aes(x = land.use.hist, y = rle12)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
  labs(title = "Previous landuse")
ggplot(data_all, aes(x = rest.age, y = rle12)) +
  geom_point() + geom_smooth(method = "glm") +
  labs(title = "Age of Restoration")
ggplot(data_all, aes(x = region, y = rle12)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
  labs(title = "Region")
ggplot(data_all, aes(x = hydrology, y = rle12)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
...
fit.norm$aic
fit.gamma$aic
# beta distribution has lowest AIC
...
# TODO needs to be checked (2025-01-30)

# check for possible interactions between covariates with coplot
coplot(rle12 ~ rest.age | region * hydrology,
       data = data_all,
...
coplot(rle12 ~ rest.age | rest.meth * land.use.hist,
       data = data_all,
       ylab = "Total species richness",
       xlab = "",
       panel = function(x, y, ...) {
...
ggplot(data_all, aes(rest.age, rle12, color = rest.meth)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F) +
  facet_wrap(~land.use.hist)
ggplot(data_all, aes(rest.age, rle12, color = hydrology)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F) +
  facet_wrap(~region)
...
interaction.plot(x.factor = data_all$rest.meth, trace.factor = data_all$land.use.hist,
                 response = data_all$rle12)
ggplot(data_all, aes(x = interaction(land.use.hist, rest.meth), y = rle12)) + geom_boxplot()
...
interaction.plot(x.factor = data_all$rest.meth, trace.factor = data_all$region,
                 response = data_all$rle12)
ggplot(data_all, aes(x = interaction(region, rest.meth), y = rle12)) + geom_boxplot()
# no clear interaction

interaction.plot(x.factor = data_all$rest.meth, trace.factor = data_all$hydrology,
                 response = data_all$rle12)
ggplot(data_all, aes(x = interaction(rest.meth, hydrology), y = rle12)) + geom_boxplot()
# interaction


## land.use.hist vs. X

ggplot(data_all, aes(rest.age, rle12, color = land.use.hist)) +
  geom_point() + theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F)
# no clear interaction
...
interaction.plot(x.factor = data_all$land.use.hist, trace.factor = data_all$region,
                 response = data_all$rle12)
ggplot(data_all, aes(x = interaction(region, land.use.hist), y = rle12)) + geom_boxplot()
# interaction

interaction.plot(x.factor = data_all$land.use.hist, trace.factor = data_all$hydrology,
                 response = data_all$rle12)
ggplot(data_all, aes(x = interaction(hydrology, land.use.hist), y = rle12)) + geom_boxplot()
# no clear interaction
...
ggplot(data_all, aes(rest.age, rle12, color = region)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F)
...
interaction.plot(x.factor = data_all$hydrology, trace.factor = data_all$region,
                 response = data_all$rle12)
ggplot(data_all, aes(x = interaction(region, hydrology), y = rle12)) + geom_boxplot()
# interaction
...
int_model <- glm(rle12 ~ rest.meth * land.use.hist, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction between rest.meth and land.use.hist

int_model <- glm(rle12 ~ rest.meth * rest.age, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction

int_model <- glm(rle12 ~ rest.meth * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction

int_model <- glm(rle12 ~ rest.meth * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> interaction
...
int_model <- glm(rle12 ~ land.use.hist * rest.age, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction 

int_model <- glm(rle12 ~ land.use.hist * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction

int_model <- glm(rle12 ~ land.use.hist * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction
...
int_model <- glm(rle12 ~ rest.age * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction between rest.age and region

int_model <- glm(rle12 ~ rest.age * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
...
int_model <- glm(rle12 ~ region * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction
...
#' unclearity if there is a linear relationship between rest.age and rle12
#' interaction between:
...
R1 <- glmmTMB(rle12 ~ 1 + (1|region), data = data_model, family = beta_family(link = "logit"))
R2 <- glmmTMB(rle12 ~ 1 + (1|hydrology), data = data_model, family = beta_family(link = "logit"))
R3 <- glmmTMB(rle12 ~ 1 + (1|region) + (1|hydrology), data = data_model, family = beta_family(link = "logit"))
R4 <- glmmTMB(rle12 ~ 1 + (1|region) + (1|region:hydrology), data = data_model, family = beta_family(link = "logit"))
R5 <- glmmTMB(rle12 ~ 1 + (1|hydrology) + (1|region:hydrology), data = data_model, family = beta_family(link = "logit"))
R6 <- glmmTMB(rle12 ~ 1 + (1|region) + (1|hydrology) + (1|region:hydrology), data = data_model, family = beta_family(link = "logit"))
R7 <- glmmTMB(rle12 ~ 1 + (rest.age.std|region) + (1|hydrology), data = data_model, family = beta_family(link = "logit"))
# -> singularity
R8 <- glmmTMB(rle12 ~ 1 + (1|region) + (rest.age.std|hydrology), data = data_model, family = beta_family(link = "logit"))
# -> singularity
R9 <- glmmTMB(rle12 ~ 1 + (rest.age.std|region) + (rest.age.std|hydrology), data = data_model, family = beta_family(link = "logit"))
# -> singularity
Rnull <- glm(rle12 ~ 1, data = data_model, family = beta_family(link = "logit")) # right family??
...
# R10 <- glmer.nb(rle12 ~ 1 + (1|region) + hydrology, data = data_model)
# AICc(R3, R10)
...
# glmm_hydr <- glmer.nb(rle12 ~ hydrology + (1|region), data = data_model)
# glmm_test <- glmer.nb(rle12 ~ rest.meth + hydrology + (1|region), data = data_model)
# glmm_test_1 <- glmer.nb(rle12 ~ rest.meth : hydrology + (1|region), data = data_model)
# glmm_test_2 <- glmer.nb(rle12 ~ rest.meth * hydrology + (1|region), data = data_model)
# summary(glmm_hydr)
# summary(glmm_test)
# summary(glmm_test_1)
# summary(glmm_test_2)
...
data_model_20y <- data_all %>%
  filter(!id.site %in% c("N_DAM", "S_JAN") ) %>% # filter outliers
  filter(rest.age <= 20)
...
yB1 <- glmmTMB(rle12 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
...
yB1_sl1 <- glmmTMB(rle12 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (rest.age.std|region) + (1|hydrology), 
                   data = data_model_20y,
                   family = beta_family(link = "logit")
)
# --> warning: singular convergence
yB1_sl2 <- glmmTMB(rle12 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (1|region) + (rest.age.std|hydrology), 
                   data = data_model_20y,
                   family = beta_family(link = "logit")
)
# --> warning: singular convergence
yB1_sl3 <- glmmTMB(rle12 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (0 + rest.age.std|region) + (1|hydrology), 
                   data = data_model_20y,
                   family = beta_family(link = "logit")
)
yB1_sl4 <- glmmTMB(rle12 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
...
# yB1a <- glmer.nb(rle12 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist 
#                  + (1|region) + (1|hydrology), 
#                    data = data_model_20y)
# 
# full_model <- yB1a
...
# check for need of random slope
yB1_drop_sl1 <- glmmTMB(rle12 ~ rest.meth + land.use.hist + rest.age.std
                        + (rest.age.std|region) + (1|hydrology), 
                        data = data_model_20y,
                        family = beta_family(link = "logit")
)
# Model convergence problem
yB1_drop_sl2 <- glmmTMB(rle12 ~ rest.meth + land.use.hist + rest.age.std
                        + (1|region) + (rest.age.std|hydrology), 
                        data = data_model_20y,
                        family = beta_family(link = "logit")
)
# Model convergence problem
yB1_drop_sl3 <- glmmTMB(rle12 ~ rest.meth + land.use.hist + rest.age.std
                        + (0 + rest.age.std|region) + (1|hydrology), 
                        data = data_model_20y,
                        family = beta_family(link = "logit")
)
yB1_drop_sl4 <- glmmTMB(rle12 ~ rest.meth + land.use.hist + rest.age.std
                        + (1|region) + (0 + rest.age.std|hydrology), 
                        data = data_model_20y,
                        family = beta_family(link = "logit")
)
...
# the model we use is:
# rle12 ~ rest.meth + land.use.hist + rest.age.std + (1 |region) + (1 |hydrology)

# load(file = here("outputs", "models", "vegetation","model_plants_restfact_hill0_yB1_final.Rdata"))


data_model_rle12_20y <- data_all %>%
  filter(!id.site %in% c("N_DAM", "S_JAN") ) %>% # filter outliers
  filter(rest.age <= 20)

restfact_rle12_20y <- glmmTMB(rle12 ~ rest.meth + land.use.hist + rest.age.std
                                 + (1 |region) + (1 |hydrology),
                                 data = data_model_rle12_20y,
                                 family = beta_family(link = "logit")
)
summary(restfact_rle12_20y)
...
performance(restfact_rle12_20y)
#' ICC = 0.222
...
model_summary <- broom.mixed::tidy(restfact_rle12_20y, effects = "fixed") %>%
...
model_summary %>% 
  write_csv(
    here(
      "outputs", "tables", "vegetation", "model_summary_restfact_rle12_20y.csv"
    )
  )
...
save(restfact_rle12_20y, data_model_rle12_20y,
     file = here("outputs", "models", "vegetation",
                 "model_plants_restfact_rle12_20y.Rdata"))
...
B1 <- glmmTMB(rle12 ~ rest.meth * land.use.hist
              + (1|region) + (1|hydrology), 
              data = data_model,
              family = beta_family(link = "logit")
)
...
# 
# # use glmer.nb() for dredge
# B1a <- glmer.nb(rle12 ~ rest.meth * land.use.hist 
#                 + (1|region) + (1|hydrology), 
#                 data = data_model)
# 
# full_model <- B1a
...
# top_model <- get.models(model_dredge, subset = 1)[[1]]
# summary(top_model)
# # rle12 ~ land.use.hist + rest.meth + (1 | region) + (1 | hydrology)
# 
# options(na.action = "na.omit") # set back to default
...
# the model we use is:
# rle12 ~ rest.meth + land.use.hist + (1 |region) + (1 |hydrology)

# load(file = here("outputs", "models", "vegetation", "model_plants_restfact_hill0_B1_final.Rdata"))

data_model_rle12 <- data_all %>%
  filter(!id.site %in% c("N_DAM", "S_JAN") ) %>% # filter outliers
  filter(!is.na(rest.meth))

restfact_rle12 <- glmmTMB(rle12 ~ rest.meth + land.use.hist
                             + (1 |region) + (1 |hydrology),
                             data = data_model_rle12,
                             family = beta_family(link = "logit")
)
summary(restfact_rle12)
# Estimate Std. Error z value Pr(>|z|)    
# (Intercept)              1.3310     0.1486   8.959   <2e-16 ***
# rest.methmga             0.3144     0.1591   1.976   0.0482 *  
# rest.methres             0.2426     0.1243   1.951   0.0510 .  
# rest.methdih             0.1970     0.1321   1.491   0.1359    
# land.use.histgrassland  -0.1297     0.1036  -1.252   0.2106 
...
emm.rest.meth <- emmeans(restfact_rle12, ~ rest.meth)
emm.rest.meth
summary(emm.rest.meth, infer = F, type = "response")
# rest.meth response     SE  df
# cus          0.780 0.0266 Inf
# mga          0.829 0.0223 Inf
# res          0.819 0.0207 Inf
# dih          0.812 0.0211 Inf
# 
...
# cus - mga  -0.3144 0.159 Inf  -1.976  0.1972
# cus - res  -0.2426 0.124 Inf  -1.951  0.2068
# cus - dih  -0.1970 0.132 Inf  -1.491  0.4427
# mga - res   0.0718 0.142 Inf   0.505  0.9580
# mga - dih   0.1174 0.147 Inf   0.799  0.8551
# res - dih   0.0457 0.114 Inf   0.401  0.9782
# 
# Results are averaged over the levels of: land.use.hist 
...
# Tidy up the model summary
model_summary <- broom.mixed::tidy(restfact_rle12, effects = "fixed") %>%
  select(term, estimate, std.error, statistic, p.value) %>%
  mutate(across(c(estimate, std.error, statistic), round, 3)) %>%   # Rounding values
  mutate(p.value = label_number(accuracy = 0.0001)(p.value)) %>% 
  mutate(p.value = case_when(p.value < 0.001 ~ "< 0.001",
...
model_summary %>% 
  write_csv(
    here(
      "outputs", "tables", "vegetation", "model_summary_restfact_rle12.csv"
    )
  )
...
data_model_rle12 %>%
  group_by(rest.meth) %>%
  get_summary_stats(rle12, type = "full")

### d save final model ----


save(restfact_rle12, data_model_rle12,
     file = here("outputs", "models", "vegetation",
                 "model_plants_restfact_rle12.Rdata"))
...
glmm_rest_age <- glmer.nb(rle12 ~ rest.age + (1|region) + (1|hydrology), data = data_model)
summary(glmm_rest_age)

glmm_rest_5 <- glmer.nb(rle12 ~
                          rest.meth
                        * rest.age
                        + (1|region) + (1|hydrology)
                        , data = data_model
...
# Finale Grafik ---
ggplot(data_model, aes(rest.age, rle12, color = region)) +
  geom_point() +
  theme_bw() +
...
top_most_models
# second best model (by AICc) is rle12 ~ land.use.hist + rest.meth + (1 | region) + (1 | hydrology)
# AIC 821.2433
...
B1a <- glm(rle12 ~ rest.meth, data = data_model, family = "poisson")
...
glmm_dih_age <- glmer.nb(rle12 ~ rest.age + (1|region) + (1|hydrology), data = data_dih)
glmm_mga_age <- glmer.nb(rle12 ~ rest.age + (1|region), data = data_mga)
glmm_res_age <- glmer.nb(rle12 ~ rest.age + (1|region), data = data_res)
glmm_cus_age <- glmer.nb(rle12 ~ rest.age + (1|region), data = data_cus)
summary(glmm_dih_age)
summary(glmm_mga_age)
summary(glmm_res_age)
summary(glmm_cus_age)

plot(data_all$rle12 ~ data_all$rest.age)

glmm_dry_age <- glmer.nb(rle12 ~ rest.age + (1|region), data = data_dry)
glmm_fresh_age <- glmer.nb(rle12 ~ rest.age + (1|region), data = data_fresh)
glmm_moist_age <- glmer.nb(rle12 ~ rest.age + (1|region), data = data_moist)
