...
(p1 <- ggplot(
  data_model_tothill0, aes(x = site.type, y = tot.hill.0, fill = site.type)
    ...
    geom_text(
      data = cld_results, aes(x = site.type, y = 85, label = .group),
...
(p2 <- ggplot(
  ...
  aes(x = site.type, y = target.hill.0, fill = site.type)
  ) +
    gglayer_theme +
    gglayer_labs +
    geom_violin(trim = TRUE, width = 0.8, linewidth = 0.5) +
...
(p3 <- ggplot(
  data_model_tothill1, aes(x = site.type, y = tot.hill.1, fill = site.type)
    ...
    geom_text(
      data = cld_results, aes(x = site.type, y = 41, label = .group),
      vjust = 0, hjust = 0.4, size = rel(4),
      family = "Franklin Gothic Book", color = "black"
    ) +
    labs(
...
(p5 <- ggplot(
  data_model_fcsihill0, aes(x = site.type, y = fcsi.hill.0, fill = site.type)
    ...
    geom_text(
      data = cld_results, aes(x = site.type, y = 4.3, label = .group),
      vjust = 0, hjust = 0.4, size = rel(4),
      family = "Franklin Gothic Book", color = "black"
    ) +
    labs(
