...
# Response variable: Difference Total Hill numbers q1-q0 (totdiffq1q0)
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
diversity <- diversity %>% 
  mutate(diff.q1.q0 = tot.hill.1 - tot.hill.0)
...
#   geom_quasirandom()
# (plot2 <- ggplot(data_all, aes(x = diff.q1.q0)) +
#   geom_histogram(binwidth = 1))
# plot3 <- ggplot(data_all, aes(x = diff.q1.q0)) +
#   geom_density()
# plot4 <- ggplot(data_all, aes(x = log(diff.q1.q0))) +
#   geom_density()
...
library(rstatix)
data_all %>% 
  select(id.site, diff.q1.q0) %>% 
  identify_outliers(diff.q1.q0)
# no extreme outliers
data_all %>% 
...
ggplot(data_all, aes(x = rest.meth, y = diff.q1.q0)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
  labs(title = "Restoration method")
ggplot(data_all, aes(x = land.use.hist, y = diff.q1.q0)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
  labs(title = "Previous landuse")
ggplot(data_all, aes(x = rest.age, y = diff.q1.q0)) +
  geom_point() + geom_smooth(method = "glm") +
  labs(title = "Age of Restoration")
ggplot(data_all, aes(x = region, y = diff.q1.q0)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
  labs(title = "Region")
ggplot(data_all, aes(x = hydrology, y = diff.q1.q0)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
...
# TODO needs to be checked (2025-01-09)

# check for possible interactions between covariates with coplot
coplot(diff.q1.q0 ~ rest.age | region * hydrology,
       data = data_all,
...
coplot(diff.q1.q0 ~ rest.age | rest.meth * land.use.hist,
       data = data_all,
       ylab = "Total species richness",
       xlab = "",
       panel = function(x, y, ...) {
...
ggplot(data_all, aes(rest.age, diff.q1.q0, color = rest.meth)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F) +
  facet_wrap(~land.use.hist)
ggplot(data_all, aes(rest.age, diff.q1.q0, color = hydrology)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F) +
  facet_wrap(~region)
...
interaction.plot(x.factor = data_all$rest.meth, trace.factor = data_all$land.use.hist,
                 response = data_all$diff.q1.q0)
ggplot(data_all, aes(x = interaction(land.use.hist, rest.meth), y = diff.q1.q0)) + geom_boxplot()
...
interaction.plot(x.factor = data_all$rest.meth, trace.factor = data_all$region,
                 response = data_all$diff.q1.q0)
ggplot(data_all, aes(x = interaction(region, rest.meth), y = diff.q1.q0)) + geom_boxplot()
# no clear interaction

interaction.plot(x.factor = data_all$rest.meth, trace.factor = data_all$hydrology,
                 response = data_all$diff.q1.q0)
ggplot(data_all, aes(x = interaction(rest.meth, hydrology), y = diff.q1.q0)) + geom_boxplot()
# interaction


## land.use.hist vs. X

ggplot(data_all, aes(rest.age, diff.q1.q0, color = land.use.hist)) +
  geom_point() + theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F)
# no clear interaction
...
interaction.plot(x.factor = data_all$land.use.hist, trace.factor = data_all$region,
                 response = data_all$diff.q1.q0)
ggplot(data_all, aes(x = interaction(region, land.use.hist), y = diff.q1.q0)) + geom_boxplot()
# interaction

interaction.plot(x.factor = data_all$land.use.hist, trace.factor = data_all$hydrology,
                 response = data_all$diff.q1.q0)
ggplot(data_all, aes(x = interaction(hydrology, land.use.hist), y = diff.q1.q0)) + geom_boxplot()
# no clear interaction
...
ggplot(data_all, aes(rest.age, diff.q1.q0, color = region)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F)
...
interaction.plot(x.factor = data_all$hydrology, trace.factor = data_all$region,
                 response = data_all$diff.q1.q0)
ggplot(data_all, aes(x = interaction(region, hydrology), y = diff.q1.q0)) + geom_boxplot()
# interaction
...
int_model <- glm(diff.q1.q0 ~ rest.meth * land.use.hist, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction between rest.meth and land.use.hist

int_model <- glm(diff.q1.q0 ~ rest.meth * rest.age, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction

int_model <- glm(diff.q1.q0 ~ rest.meth * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction

int_model <- glm(diff.q1.q0 ~ rest.meth * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> interaction
...
int_model <- glm(diff.q1.q0 ~ land.use.hist * rest.age, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction 

int_model <- glm(diff.q1.q0 ~ land.use.hist * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction

int_model <- glm(diff.q1.q0 ~ land.use.hist * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction
...
int_model <- glm(diff.q1.q0 ~ rest.age * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction between rest.age and region

int_model <- glm(diff.q1.q0 ~ rest.age * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
...
int_model <- glm(diff.q1.q0 ~ region * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction
...
#' unclearity if there is a linear relationship between rest.age and diff.q1.q0
#' interaction between:
...
R1 <- glmmTMB(diff.q1.q0 ~ 1 + (1|region), data = data_model, family = gaussian)
R2 <- glmmTMB(diff.q1.q0 ~ 1 + (1|hydrology), data = data_model, family = gaussian)
R3 <- glmmTMB(diff.q1.q0 ~ 1 + (1|region) + (1|hydrology), data = data_model, family = gaussian)
R4 <- glmmTMB(diff.q1.q0 ~ 1 + (1|region) + (1|region:hydrology), data = data_model, family = gaussian)
R5 <- glmmTMB(diff.q1.q0 ~ 1 + (1|hydrology) + (1|region:hydrology), data = data_model, family = gaussian)
R6 <- glmmTMB(diff.q1.q0 ~ 1 + (1|region) + (1|hydrology) + (1|region:hydrology), data = data_model, family = gaussian)
R7 <- glmmTMB(diff.q1.q0 ~ 1 + (rest.age.std|region) + (1|hydrology), data = data_model, family = gaussian)
# -> singularity
R8 <- glmmTMB(diff.q1.q0 ~ 1 + (1|region) + (rest.age.std|hydrology), data = data_model, family = gaussian)
# -> singularity
R9 <- glmmTMB(diff.q1.q0 ~ 1 + (rest.age.std|region) + (rest.age.std|hydrology), data = data_model, family = gaussian)
# -> singularity
Rnull <- glm(diff.q1.q0 ~ 1, data = data_model, family = gaussian) # right family??
...
# R10 <- glmer.nb(diff.q1.q0 ~ 1 + (1|region) + hydrology, data = data_model)
# AICc(R3, R10)
...
# glmm_hydr <- glmer.nb(diff.q1.q0 ~ hydrology + (1|region), data = data_model)
# glmm_test <- glmer.nb(diff.q1.q0 ~ rest.meth + hydrology + (1|region), data = data_model)
# glmm_test_1 <- glmer.nb(diff.q1.q0 ~ rest.meth : hydrology + (1|region), data = data_model)
# glmm_test_2 <- glmer.nb(diff.q1.q0 ~ rest.meth * hydrology + (1|region), data = data_model)
# summary(glmm_hydr)
# summary(glmm_test)
# summary(glmm_test_1)
# summary(glmm_test_2)
...
yB1 <- glmmTMB(diff.q1.q0 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
...
yB1_sl1 <- glmmTMB(diff.q1.q0 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (rest.age.std|region) + (1|hydrology), 
                   data = data_model_20y,
                   family = gaussian
)
# --> warning: singular convergence
yB1_sl2 <- glmmTMB(diff.q1.q0 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (1|region) + (rest.age.std|hydrology), 
                   data = data_model_20y,
                   family = gaussian
)
# --> warning: singular convergence
yB1_sl3 <- glmmTMB(diff.q1.q0 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (0 + rest.age.std|region) + (1|hydrology), 
                   data = data_model_20y,
                   family = gaussian
)
yB1_sl4 <- glmmTMB(diff.q1.q0 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
...
# yB1a <- glmer.nb(diff.q1.q0 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist 
#                  + (1|region) + (1|hydrology), 
#                    data = data_model_20y)
# 
# full_model <- yB1a
...
# check for need of random slope
yB1_drop_sl1 <- glmmTMB(diff.q1.q0 ~ rest.meth + land.use.hist + rest.age.std
                        + (rest.age.std|region) + (1|hydrology), 
                        data = data_model_20y,
                        family = gaussian
)
# Model convergence problem
yB1_drop_sl2 <- glmmTMB(diff.q1.q0 ~ rest.meth + land.use.hist + rest.age.std
                        + (1|region) + (rest.age.std|hydrology), 
                        data = data_model_20y,
                        family = gaussian
)
# Model convergence problem
yB1_drop_sl3 <- glmmTMB(diff.q1.q0 ~ rest.meth + land.use.hist + rest.age.std
                        + (0 + rest.age.std|region) + (1|hydrology), 
                        data = data_model_20y,
                        family = gaussian
)
yB1_drop_sl4 <- glmmTMB(diff.q1.q0 ~ rest.meth + land.use.hist + rest.age.std
                        + (1|region) + (0 + rest.age.std|hydrology), 
                        data = data_model_20y,
                        family = gaussian
)
...
# the model we use is:
# diff.q1.q0 ~ rest.meth + land.use.hist + rest.age.std + (1 |region) + (1 |hydrology)

# load(file = here("outputs", "models", "vegetation","model_plants_restfact_hill0_yB1_final.Rdata"))


data_model_diffq1q0_20y <- data_all %>%
  filter(rest.age <= 20)

restfact_diffq1q0_20y <- glmmTMB(diff.q1.q0 ~ rest.meth + land.use.hist + rest.age.std
                                + (1 |region) + (1 |hydrology),
                                data = data_model_diffq1q0_20y,
                                family = gaussian
)
summary(restfact_diffq1q0_20y)
# Estimate Std. Error z value Pr(>|z|)    
# (Intercept)            -21.3592     3.5803  -5.966 2.43e-09 ***
#   rest.methmga             0.2128     3.8820   0.055  0.95628    
# rest.methres            -2.9099     3.1446  -0.925  0.35478    
# rest.methdih            -8.9515     3.3329  -2.686  0.00724 ** 
#   land.use.histgrassland  -2.6228     1.9898  -1.318  0.18745    
# rest.age.std            -0.1833     1.5273  -0.120  0.90446  

# --> restoration age is not significant

performance(restfact_diffq1q0_20y)
#' ICC = 0.222
#' The covariates explain 19 % of the variation in richness (R2 marg.)
#' The covariates and random effects explain 37 % of the variation. (R2 cond.)


### b report final model ----

# Tidy up the model summary
model_summary <- broom.mixed::tidy(restfact_diffq1q0_20y, effects = "fixed") %>%
...
model_summary %>% 
  write_csv(
    here(
      "outputs", "tables", "vegetation", "model_summary_restfact_diffq1q0_20y.csv"
    )
  )
...
save(restfact_diffq1q0_20y, data_model_diffq1q0_20y,
     file = here("outputs", "models", "vegetation",
                 "model_plants_restfact_diffq1q0_20y.Rdata"))
...
B1 <- glmmTMB(diff.q1.q0 ~ rest.meth * land.use.hist
              + (1|region) + (1|hydrology), 
              data = data_model,
              family = gaussian
)
...
# 
# # use glmer.nb() for dredge
# B1a <- glmer.nb(diff.q1.q0 ~ rest.meth * land.use.hist 
#                 + (1|region) + (1|hydrology), 
#                 data = data_model)
# 
# full_model <- B1a
...
# top_model <- get.models(model_dredge, subset = 1)[[1]]
# summary(top_model)
# # diff.q1.q0 ~ land.use.hist + rest.meth + (1 | region) + (1 | hydrology)
# 
# options(na.action = "na.omit") # set back to default
...
ggplot(data = data_model, aes(x = rest.age.std, y = E_B1_drop)) +
  ...
  labs(x = "values", y = "Residuals") + 
  geom_hline(yintercept = 0, lty = 2) 
#' maller variance in higher values
...
# the model we use is:
# diff.q1.q0 ~ rest.meth + land.use.hist + (1 |region) + (1 |hydrology)

# load(file = here("outputs", "models", "vegetation", "model_plants_restfact_hill0_B1_final.Rdata"))

data_model_diffq1q0 <- data_all %>%
  filter(!is.na(rest.meth))

restfact_diffq1q0 <- glmmTMB(diff.q1.q0 ~ rest.meth + land.use.hist
                            + (1 |region) + (1 |hydrology),
                            data = data_model_diffq1q0,
                            family = gaussian
)
summary(restfact_diffq1q0)
# Estimate Std. Error z value Pr(>|z|)    
# (Intercept)            -21.82602    2.83918  -7.687  1.5e-14 ***
#   rest.methmga            -0.09572    2.52408  -0.038 0.969750    
# rest.methres            -2.88917    1.84547  -1.566 0.117455    
# rest.methdih            -7.52243    2.01636  -3.731 0.000191 ***
#   land.use.histgrassland  -2.88544    1.50599  -1.916 0.055369 .  
...
# cus - mga   0.0957 2.52 113   0.038  1.0000
# cus - res   2.8892 1.85 113   1.566  0.4024
# cus - dih   7.5224 2.02 113   3.731  0.0017
# mga - res   2.7935 2.27 113   1.233  0.6071
# mga - dih   7.4267 2.16 113   3.440  0.0045
# res - dih   4.6333 1.73 113   2.672  0.0424
# 
...
# cus - dih  -0.5211 0.240 Inf  -2.174  0.1306
# cus - mga  -0.0335 0.204 Inf  -0.164  0.9984
# cus - res  -0.5837 0.265 Inf  -2.205  0.1220
# dih - mga   0.4876 0.252 Inf   1.939  0.2119
# dih - res  -0.0625 0.259 Inf  -0.241  0.9951
# mga - res  -0.5501 0.294 Inf  -1.872  0.2401
# 
# Results are averaged over the levels of: land.use.hist 
# P value adjustment: tukey method for comparing a family of 4 estimates 
...
# Tidy up the model summary
model_summary <- broom.mixed::tidy(restfact_diffq1q0, effects = "fixed") %>%
  select(term, estimate, std.error, statistic, p.value) %>%
  mutate(across(c(estimate, std.error, statistic), round, 3)) %>%   # Rounding values
  mutate(p.value = label_number(accuracy = 0.0001)(p.value)) %>% 
  mutate(p.value = case_when(p.value < 0.001 ~ "< 0.001",
...
model_summary %>% 
  write_csv(
    here(
      "outputs", "tables", "vegetation", "model_summary_restfact_diffq1q0.csv"
    )
  )
...
data_model_diffq1q0 %>%
  group_by(rest.meth) %>%
  get_summary_stats(diff.q1.q0, type = "full")

### d save final model ----


save(restfact_diffq1q0, data_model_diffq1q0,
     file = here("outputs", "models", "vegetation",
                 "model_plants_restfact_diffq1q0.Rdata"))
...
glmm_rest_age <- glmer.nb(diff.q1.q0 ~ rest.age + (1|region) + (1|hydrology), data = data_model)
summary(glmm_rest_age)

glmm_rest_5 <- glmer.nb(diff.q1.q0 ~
                          rest.meth
                        * rest.age
                        + (1|region) + (1|hydrology)
                        , data = data_model
...
# Finale Grafik ---
ggplot(data_model, aes(rest.age, diff.q1.q0, color = region)) +
  geom_point() +
  theme_bw() +
...
B1_gamm <- gamm(
  diff.q1.q0 ~ rest.meth + land.use.hist + 
...
top_most_models
# second best model (by AICc) is diff.q1.q0 ~ land.use.hist + rest.meth + (1 | region) + (1 | hydrology)
# AIC 821.2433
...
B1a <- glm(diff.q1.q0 ~ rest.meth, data = data_model, family = "poisson")
...
glmm_dih_age <- glmer.nb(diff.q1.q0 ~ rest.age + (1|region) + (1|hydrology), data = data_dih)
glmm_mga_age <- glmer.nb(diff.q1.q0 ~ rest.age + (1|region), data = data_mga)
glmm_res_age <- glmer.nb(diff.q1.q0 ~ rest.age + (1|region), data = data_res)
glmm_cus_age <- glmer.nb(diff.q1.q0 ~ rest.age + (1|region), data = data_cus)
summary(glmm_dih_age)
summary(glmm_mga_age)
summary(glmm_res_age)
summary(glmm_cus_age)

plot(data_all$diff.q1.q0 ~ data_all$rest.age)

glmm_dry_age <- glmer.nb(diff.q1.q0 ~ rest.age + (1|region), data = data_dry)
glmm_fresh_age <- glmer.nb(diff.q1.q0 ~ rest.age + (1|region), data = data_fresh)
glmm_moist_age <- glmer.nb(diff.q1.q0 ~ rest.age + (1|region), data = data_moist)
