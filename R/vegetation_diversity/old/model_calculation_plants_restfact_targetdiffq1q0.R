...
# Response variable: Difference Characteristic Hill numbers q1-q0 (targettargetdiffq1q0)
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
diversity <- read_csv(
  here("data", "processed", "data_processed_plants_site_diversity_20241127.csv"),
  col_names = TRUE, na = c("na", "NA", ""), col_types = cols(
    .default = "?"
  ))

diversity <- diversity %>% 
  mutate(target.target.diff.q1.q0 = target.hill.1 - target.hill.0)
...
#   geom_quasirandom()
# (plot2 <- ggplot(data_all, aes(x = target.diff.q1.q0)) +
#   geom_histogram(binwidth = 1))
# plot3 <- ggplot(data_all, aes(x = target.diff.q1.q0)) +
#   geom_density()
# plot4 <- ggplot(data_all, aes(x = log(target.diff.q1.q0))) +
#   geom_density()
...
sort(data_all$target.diff.q1.q0)
...
library(rstatix)
data_all %>% 
  select(id.site, target.diff.q1.q0) %>% 
  identify_outliers(target.diff.q1.q0)
# no extreme outliers
data_all %>% 
...
ggplot(data_all, aes(x = rest.meth, y = target.diff.q1.q0)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
  labs(title = "Restoration method")
ggplot(data_all, aes(x = land.use.hist, y = target.diff.q1.q0)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
  labs(title = "Previous landuse")
ggplot(data_all, aes(x = rest.age, y = target.diff.q1.q0)) +
  geom_point() + geom_smooth(method = "glm") +
  labs(title = "Age of Restoration")
ggplot(data_all, aes(x = region, y = target.diff.q1.q0)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
  labs(title = "Region")
ggplot(data_all, aes(x = hydrology, y = target.diff.q1.q0)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
...
## f distribution --------------------------------------------------------------
library(lattice)
histogram(data_all$target.diff.q1.q0)
# normal distribution
...
# TODO needs to be checked (2025-01-17)

# check for possible interactions between covariates with coplot
coplot(target.diff.q1.q0 ~ rest.age | region * hydrology,
       data = data_all,
...
coplot(target.diff.q1.q0 ~ rest.age | rest.meth * land.use.hist,
       data = data_all,
       ylab = "Total species richness",
       xlab = "",
       panel = function(x, y, ...) {
...
ggplot(data_all, aes(rest.age, target.diff.q1.q0, color = rest.meth)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F) +
  facet_wrap(~land.use.hist)
ggplot(data_all, aes(rest.age, target.diff.q1.q0, color = hydrology)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F) +
  facet_wrap(~region)
...
interaction.plot(x.factor = data_all$rest.meth, trace.factor = data_all$land.use.hist,
                 response = data_all$target.diff.q1.q0)
ggplot(data_all, aes(x = interaction(land.use.hist, rest.meth), y = target.diff.q1.q0)) + geom_boxplot()
...
interaction.plot(x.factor = data_all$rest.meth, trace.factor = data_all$region,
                 response = data_all$target.diff.q1.q0)
ggplot(data_all, aes(x = interaction(region, rest.meth), y = target.diff.q1.q0)) + geom_boxplot()
# no clear interaction

interaction.plot(x.factor = data_all$rest.meth, trace.factor = data_all$hydrology,
                 response = data_all$target.diff.q1.q0)
ggplot(data_all, aes(x = interaction(rest.meth, hydrology), y = target.diff.q1.q0)) + geom_boxplot()
# interaction


## land.use.hist vs. X

ggplot(data_all, aes(rest.age, target.diff.q1.q0, color = land.use.hist)) +
  geom_point() + theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F)
# no clear interaction
...
interaction.plot(x.factor = data_all$land.use.hist, trace.factor = data_all$region,
                 response = data_all$target.diff.q1.q0)
ggplot(data_all, aes(x = interaction(region, land.use.hist), y = target.diff.q1.q0)) + geom_boxplot()
# interaction

interaction.plot(x.factor = data_all$land.use.hist, trace.factor = data_all$hydrology,
                 response = data_all$target.diff.q1.q0)
ggplot(data_all, aes(x = interaction(hydrology, land.use.hist), y = target.diff.q1.q0)) + geom_boxplot()
# no clear interaction
...
ggplot(data_all, aes(rest.age, target.diff.q1.q0, color = region)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F)
...
interaction.plot(x.factor = data_all$hydrology, trace.factor = data_all$region,
                 response = data_all$target.diff.q1.q0)
ggplot(data_all, aes(x = interaction(region, hydrology), y = target.diff.q1.q0)) + geom_boxplot()
# interaction
...
int_model <- glm(target.diff.q1.q0 ~ rest.meth * land.use.hist, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction between rest.meth and land.use.hist

int_model <- glm(target.diff.q1.q0 ~ rest.meth * rest.age, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction

int_model <- glm(target.diff.q1.q0 ~ rest.meth * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction

int_model <- glm(target.diff.q1.q0 ~ rest.meth * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> interaction
...
int_model <- glm(target.diff.q1.q0 ~ land.use.hist * rest.age, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction 

int_model <- glm(target.diff.q1.q0 ~ land.use.hist * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction

int_model <- glm(target.diff.q1.q0 ~ land.use.hist * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction
...
int_model <- glm(target.diff.q1.q0 ~ rest.age * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction between rest.age and region

int_model <- glm(target.diff.q1.q0 ~ rest.age * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
...
int_model <- glm(target.diff.q1.q0 ~ region * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction
...
#' unclearity if there is a linear relationship between rest.age and target.diff.q1.q0
#' interaction between:
...
R1 <- glmmTMB(target.diff.q1.q0 ~ 1 + (1|region), data = data_model, family = gaussian)
R2 <- glmmTMB(target.diff.q1.q0 ~ 1 + (1|hydrology), data = data_model, family = gaussian)
R3 <- glmmTMB(target.diff.q1.q0 ~ 1 + (1|region) + (1|hydrology), data = data_model, family = gaussian)
R4 <- glmmTMB(target.diff.q1.q0 ~ 1 + (1|region) + (1|region:hydrology), data = data_model, family = gaussian)
R5 <- glmmTMB(target.diff.q1.q0 ~ 1 + (1|hydrology) + (1|region:hydrology), data = data_model, family = gaussian)
R6 <- glmmTMB(target.diff.q1.q0 ~ 1 + (1|region) + (1|hydrology) + (1|region:hydrology), data = data_model, family = gaussian)
R7 <- glmmTMB(target.diff.q1.q0 ~ 1 + (rest.age.std|region) + (1|hydrology), data = data_model, family = gaussian)
# -> singularity
R8 <- glmmTMB(target.diff.q1.q0 ~ 1 + (1|region) + (rest.age.std|hydrology), data = data_model, family = gaussian)
# -> singularity
R9 <- glmmTMB(target.diff.q1.q0 ~ 1 + (rest.age.std|region) + (rest.age.std|hydrology), data = data_model, family = gaussian)
# -> singularity
Rnull <- glm(target.diff.q1.q0 ~ 1, data = data_model, family = gaussian) # right family??
...
# R10 <- glmer.nb(target.diff.q1.q0 ~ 1 + (1|region) + hydrology, data = data_model)
# AICc(R3, R10)
...
# glmm_hydr <- glmer.nb(target.diff.q1.q0 ~ hydrology + (1|region), data = data_model)
# glmm_test <- glmer.nb(target.diff.q1.q0 ~ rest.meth + hydrology + (1|region), data = data_model)
# glmm_test_1 <- glmer.nb(target.diff.q1.q0 ~ rest.meth : hydrology + (1|region), data = data_model)
# glmm_test_2 <- glmer.nb(target.diff.q1.q0 ~ rest.meth * hydrology + (1|region), data = data_model)
# summary(glmm_hydr)
# summary(glmm_test)
# summary(glmm_test_1)
# summary(glmm_test_2)
...
yB1 <- glmmTMB(target.diff.q1.q0 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
...
yB1_sl1 <- glmmTMB(target.diff.q1.q0 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (rest.age.std|region) + (1|hydrology), 
                   data = data_model_20y,
                   family = gaussian
)
yB1_sl2 <- glmmTMB(target.diff.q1.q0 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (1|region) + (rest.age.std|hydrology), 
                   data = data_model_20y,
                   family = gaussian
)
# --> warning: singular convergence
yB1_sl3 <- glmmTMB(target.diff.q1.q0 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (0 + rest.age.std|region) + (1|hydrology), 
                   data = data_model_20y,
                   family = gaussian
)
yB1_sl4 <- glmmTMB(target.diff.q1.q0 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
...
# yB1a <- glmer.nb(target.diff.q1.q0 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist 
#                  + (1|region) + (1|hydrology), 
#                    data = data_model_20y)
# 
# full_model <- yB1a
...
# check for need of random slope
yB1_drop_sl1 <- glmmTMB(target.diff.q1.q0 ~ rest.meth + land.use.hist + rest.age.std
                        + (rest.age.std|region) + (1|hydrology), 
                        data = data_model_20y,
                        family = gaussian
)
# Model convergence problem
yB1_drop_sl2 <- glmmTMB(target.diff.q1.q0 ~ rest.meth + land.use.hist + rest.age.std
                        + (1|region) + (rest.age.std|hydrology), 
                        data = data_model_20y,
                        family = gaussian
)
# Model convergence problem
yB1_drop_sl3 <- glmmTMB(target.diff.q1.q0 ~ rest.meth + land.use.hist + rest.age.std
                        + (0 + rest.age.std|region) + (1|hydrology), 
                        data = data_model_20y,
                        family = gaussian
)
yB1_drop_sl4 <- glmmTMB(target.diff.q1.q0 ~ rest.meth + land.use.hist + rest.age.std
                        + (1|region) + (0 + rest.age.std|hydrology), 
                        data = data_model_20y,
                        family = gaussian
)
...
# the model we use is:
# target.diff.q1.q0 ~ rest.meth + land.use.hist + rest.age.std + (1 |region) + (1 |hydrology)

# load(file = here("outputs", "models", "vegetation","model_plants_restfact_hill0_yB1_final.Rdata"))


data_model_targettargetdiffq1q0_20y <- data_all %>%
  filter(rest.age <= 20)

restfact_targettargetdiffq1q0_20y <- glmmTMB(target.diff.q1.q0 ~ rest.meth + land.use.hist + rest.age.std
                                 + (1 |region) + (1 |hydrology),
                                 data = data_model_targettargetdiffq1q0_20y,
                                 family = gaussian
)
summary(restfact_targettargetdiffq1q0_20y)
# Estimate Std. Error z value Pr(>|z|)    
# (Intercept)            -16.7205     2.9977  -5.578 2.44e-08 ***
# rest.methmga             0.6335     3.0436   0.208  0.83511    
# rest.methres            -3.6301     2.4567  -1.478  0.13950    
# rest.methdih            -7.2318     2.6220  -2.758  0.00581 ** 
# land.use.histgrassland  -2.9232     1.5692  -1.863  0.06249 .  
# rest.age.std            -0.7516     1.2003  -0.626  0.53124  

# --> restoration age is not significant

performance(restfact_targettargetdiffq1q0_20y)
#' ICC = 0.281
#' The covariates explain 19 % of the variation in richness (R2 marg.)
#' The covariates and random effects explain 41 % of the variation. (R2 cond.)


### b report final model ----

# Tidy up the model summary
model_summary <- broom.mixed::tidy(restfact_targettargetdiffq1q0_20y, effects = "fixed") %>%
...
model_summary %>% 
  write_csv(
    here(
      "outputs", "tables", "vegetation", "model_summary_restfact_targettargetdiffq1q0_20y.csv"
    )
  )
...
save(restfact_targettargetdiffq1q0_20y, data_model_targettargetdiffq1q0_20y,
     file = here("outputs", "models", "vegetation",
                 "model_plants_restfact_targettargetdiffq1q0_20y.Rdata"))
...
B1 <- glmmTMB(target.diff.q1.q0 ~ rest.meth * land.use.hist
              + (1|region) + (1|hydrology), 
              data = data_model,
              family = gaussian
)
...
# 
# # use glmer.nb() for dredge
# B1a <- glmer.nb(target.diff.q1.q0 ~ rest.meth * land.use.hist 
#                 + (1|region) + (1|hydrology), 
#                 data = data_model)
# 
# full_model <- B1a
...
# top_model <- get.models(model_dredge, subset = 1)[[1]]
# summary(top_model)
# # target.diff.q1.q0 ~ land.use.hist + rest.meth + (1 | region) + (1 | hydrology)
# 
# options(na.action = "na.omit") # set back to default
...
# the model we use is:
# target.diff.q1.q0 ~ rest.meth + land.use.hist + (1 |region) + (1 |hydrology)

# load(file = here("outputs", "models", "vegetation", "model_plants_restfact_hill0_B1_final.Rdata"))

data_model_targetdiffq1q0 <- data_all %>%
  filter(!is.na(rest.meth))

restfact_targetdiffq1q0 <- glmmTMB(target.diff.q1.q0 ~ rest.meth + land.use.hist
                             + (1 |region) + (1 |hydrology),
                             data = data_model_targetdiffq1q0,
                             family = gaussian
)
summary(restfact_targetdiffq1q0)
# Estimate Std. Error z value Pr(>|z|)    
# (Intercept)            -17.4035     2.6611  -6.540 6.16e-11 ***
# rest.methmga             0.7206     2.0401   0.353 0.723926    
# rest.methres            -3.0117     1.4884  -2.023 0.043023 *  
# rest.methdih            -5.7289     1.6279  -3.519 0.000433 ***
# land.use.histgrassland  -2.7146     1.2155  -2.233 0.025526 *  
...
# cus - mga   -0.721 2.04 113  -0.353  0.9848
# cus - res    3.012 1.49 113   2.023  0.1855
# cus - dih    5.729 1.63 113   3.519  0.0035
# mga - res    3.732 1.83 113   2.035  0.1815
# mga - dih    6.450 1.75 113   3.679  0.0020
# res - dih    2.717 1.41 113   1.931  0.2211
# 
# Results are averaged over the levels of: land.use.hist 
...
#### Post-hoc land.use.hist ####


# Tukey-adjusted pariwise comparisons
# generate estimated marginal means (EMMs) and then apply a Tukey correction to 
# pairwise comparisons
emm.land.use.hist <- emmeans(restfact_targetdiffq1q0, ~ land.use.hist)
emm.land.use.hist
summary(emm.land.use.hist, infer = F, type = "response")
# land.use.hist emmean   SE  df
# arable land    -19.4 2.45 113
# grassland      -22.1 2.54 113
# 
# Results are averaged over the levels of: rest.meth 
pairs(emm.land.use.hist, adjust = "tukey") # regrid() for back-transformation from log-scale
# contrast                estimate   SE  df t.ratio p.value
# arable land - grassland     2.71 1.22 113   2.233  0.0275
# 
# Results are averaged over the levels of: rest.meth 
...
# Tidy up the model summary
model_summary <- broom.mixed::tidy(restfact_targetdiffq1q0, effects = "fixed") %>%
  select(term, estimate, std.error, statistic, p.value) %>%
  mutate(across(c(estimate, std.error, statistic), round, 3)) %>%   # Rounding values
  mutate(p.value = label_number(accuracy = 0.0001)(p.value)) %>% 
  mutate(p.value = case_when(p.value < 0.001 ~ "< 0.001",
...
model_summary %>% 
  write_csv(
    here(
      "outputs", "tables", "vegetation", "model_summary_restfact_targetdiffq1q0.csv"
    )
  )

### summary statistics ###
data_model_targetdiffq1q0 %>%
  group_by(rest.meth) %>%
  get_summary_stats(target.diff.q1.q0, type = "full")

### d save final model ----


save(restfact_targetdiffq1q0, data_model_targetdiffq1q0,
     file = here("outputs", "models", "vegetation",
                 "model_plants_restfact_targetdiffq1q0.Rdata"))
...
glmm_rest_age <- glmer.nb(target.diff.q1.q0 ~ rest.age + (1|region) + (1|hydrology), data = data_model)
summary(glmm_rest_age)

glmm_rest_5 <- glmer.nb(target.diff.q1.q0 ~
                          rest.meth
                        * rest.age
                        + (1|region) + (1|hydrology)
                        , data = data_model
...
# Finale Grafik ---
ggplot(data_model, aes(rest.age, target.diff.q1.q0, color = region)) +
  geom_point() +
  theme_bw() +
...
B1_gamm <- gamm(
  target.diff.q1.q0 ~ rest.meth + land.use.hist + 
...
top_most_models
# second best model (by AICc) is target.diff.q1.q0 ~ land.use.hist + rest.meth + (1 | region) + (1 | hydrology)
# AIC 821.2433
...
B1a <- glm(target.diff.q1.q0 ~ rest.meth, data = data_model, family = "poisson")
...
glmm_dih_age <- glmer.nb(target.diff.q1.q0 ~ rest.age + (1|region) + (1|hydrology), data = data_dih)
glmm_mga_age <- glmer.nb(target.diff.q1.q0 ~ rest.age + (1|region), data = data_mga)
glmm_res_age <- glmer.nb(target.diff.q1.q0 ~ rest.age + (1|region), data = data_res)
glmm_cus_age <- glmer.nb(target.diff.q1.q0 ~ rest.age + (1|region), data = data_cus)
summary(glmm_dih_age)
summary(glmm_mga_age)
summary(glmm_res_age)
summary(glmm_cus_age)

plot(data_all$target.diff.q1.q0 ~ data_all$rest.age)

glmm_dry_age <- glmer.nb(target.diff.q1.q0 ~ rest.age + (1|region), data = data_dry)
glmm_fresh_age <- glmer.nb(target.diff.q1.q0 ~ rest.age + (1|region), data = data_fresh)
glmm_moist_age <- glmer.nb(target.diff.q1.q0 ~ rest.age + (1|region), data = data_moist)
