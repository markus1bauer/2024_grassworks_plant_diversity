...
#   geom_quasirandom()
# (plot2 <- ggplot(data_all, aes(x = target.hill.1)) +
#   geom_histogram(binwidth = 1))
# plot3 <- ggplot(data_all, aes(x = target.hill.1)) +
#   geom_density()
# plot4 <- ggplot(data_all, aes(x = log(target.hill.1))) +
#   geom_density()
...
ggplot(data_all, aes(x = rest.meth, y = target.hill.1)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
  labs(title = "Restoration method")
ggplot(data_all, aes(x = land.use.hist, y = target.hill.1)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
  labs(title = "Previous landuse")
ggplot(data_all, aes(x = rest.age, y = target.hill.1)) +
...
# TODO needs to be checked (2025-03-12)

# check for possible interactions between covariates with coplot
coplot(target.hill.1 ~ rest.age | region * hydrology,
       data = data_all,
...
coplot(target.hill.1 ~ rest.age | rest.meth * land.use.hist,
       data = data_all,
       ylab = "Total species richness",
       xlab = "",
       panel = function(x, y, ...) {
...
ggplot(data_all, aes(rest.age, target.hill.1, color = rest.meth)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F) +
  facet_wrap(~land.use.hist)
ggplot(data_all, aes(rest.age, target.hill.1, color = hydrology)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F) +
  facet_wrap(~region)
...
ggplot(data_all, aes(x = interaction(land.use.hist, rest.meth), y = target.hill.1)) + geom_boxplot()
# interaction
...
interaction.plot(x.factor = data_all$rest.meth, trace.factor = data_all$region,
                 response = data_all$target.hill.1)
ggplot(data_all, aes(x = interaction(region, rest.meth), y = target.hill.1)) + geom_boxplot()
# interaction

interaction.plot(x.factor = data_all$rest.meth, trace.factor = data_all$hydrology,
                 response = data_all$target.hill.1)
ggplot(data_all, aes(x = interaction(hydrology, rest.meth), y = target.hill.1)) + geom_boxplot()
# interaction


## land.use.hist vs. X

ggplot(data_all, aes(rest.age, target.hill.1, color = land.use.hist)) +
  geom_point() + theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F)
...
interaction.plot(x.factor = data_all$land.use.hist, trace.factor = data_all$region,
                 response = data_all$target.hill.1)
ggplot(data_all, aes(x = interaction(region, land.use.hist), y = target.hill.1)) + geom_boxplot()
# no interaction

interaction.plot(x.factor = data_all$land.use.hist, trace.factor = data_all$hydrology,
                 response = data_all$target.hill.1)
ggplot(data_all, aes(x = interaction(hydrology, land.use.hist), y = target.hill.1)) + geom_boxplot()
# interaction
...
ggplot(data_all, aes(rest.age, target.hill.1, color = region)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = Gamma(link = "log")), se = F)
...
interaction.plot(x.factor = data_all$hydrology, trace.factor = data_all$region,
                 response = data_all$target.hill.1)
ggplot(data_all, aes(x = interaction(region, hydrology), y = target.hill.1)) + geom_boxplot()
# no interaction
...
int_model <- glm(target.hill.1 ~ rest.meth * land.use.hist, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction between rest.meth and land.use.hist

int_model <- glm(target.hill.1 ~ rest.meth * rest.age, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction

int_model <- glm(target.hill.1 ~ rest.meth * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction

int_model <- glm(target.hill.1 ~ rest.meth * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> interaction
...
int_model <- glm(target.hill.1 ~ land.use.hist * rest.age, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# interaction 

int_model <- glm(target.hill.1 ~ land.use.hist * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction

int_model <- glm(target.hill.1 ~ land.use.hist * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> interaction
...
int_model <- glm(target.hill.1 ~ rest.age * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> interaction between rest.age and region

int_model <- glm(target.hill.1 ~ rest.age * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
...
int_model <- glm(target.hill.1 ~ region * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction
...
#' unclearity if there is a linear relationship between rest.age and target.hill.1
#' interaction between:
...
R7 <- glmmTMB(target.hill.1 ~ 1 + (rest.age.std|region) + (1|hydrology), data = data_model, family = Gamma(link="log"))
R8 <- glmmTMB(target.hill.1 ~ 1 + (1|region) + (rest.age.std|hydrology), data = data_model, family = Gamma(link="log"))
# -> singularity
R9 <- glmmTMB(target.hill.1 ~ 1 + (rest.age.std|region) + (rest.age.std|hydrology), data = data_model, family = Gamma(link="log"))
# -> singularity
...
# R10 <- glmer.nb(target.hill.1 ~ 1 + (1|region) + hydrology, data = data_model)
# AICc(R3, R10)
...
# glmm_hydr <- glmer.nb(target.hill.1 ~ hydrology + (1|region), data = data_model)
# glmm_test <- glmer.nb(target.hill.1 ~ rest.meth + hydrology + (1|region), data = data_model)
# glmm_test_1 <- glmer.nb(target.hill.1 ~ rest.meth : hydrology + (1|region), data = data_model)
# glmm_test_2 <- glmer.nb(target.hill.1 ~ rest.meth * hydrology + (1|region), data = data_model)
# summary(glmm_hydr)
# summary(glmm_test)
# summary(glmm_test_1)
# summary(glmm_test_2)
...
yB1 <- glmmTMB(target.hill.1 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
...
#' --> variance in residuals higher with lower fitted values?
...
yB1_sl1 <- glmmTMB(target.hill.1 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (rest.age.std|region) + (1|hydrology), 
                   data = data_model_20y,
                   family = Gamma(link="log")
)
# --> warning: singular convergence
yB1_sl2 <- glmmTMB(target.hill.1 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (1|region) + (rest.age.std|hydrology), 
                   data = data_model_20y,
                   family = Gamma(link="log")
)
# --> warning: singular convergence
yB1_sl3 <- glmmTMB(target.hill.1 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (0 + rest.age.std|region) + (1|hydrology), 
                   data = data_model_20y,
                   family = Gamma(link="log")
)
yB1_sl4 <- glmmTMB(target.hill.1 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
...
# yB1a <- glmer.nb(target.hill.1 ~ rest.meth * land.use.hist + rest.age.std * land.use.hist 
#                  + (1|region) + (1|hydrology), 
#                    data = data_model_20y)
# 
# full_model <- yB1a
...
# check for need of random slope
yB1_drop_sl1 <- glmmTMB(target.hill.1 ~ rest.meth + land.use.hist + rest.age.std
                        + (rest.age.std|region) + (1|hydrology), 
                        data = data_model_20y,
                        family = Gamma(link="log")
)
# Model convergence problem
yB1_drop_sl2 <- glmmTMB(target.hill.1 ~ rest.meth + land.use.hist + rest.age.std
                        + (1|region) + (rest.age.std|hydrology), 
                        data = data_model_20y,
                        family = Gamma(link="log")
)
# Model convergence problem
yB1_drop_sl3 <- glmmTMB(target.hill.1 ~ rest.meth + land.use.hist + rest.age.std
                        + (0 + rest.age.std|region) + (1|hydrology), 
                        data = data_model_20y,
                        family = Gamma(link="log")
)
yB1_drop_sl4 <- glmmTMB(target.hill.1 ~ rest.meth + land.use.hist + rest.age.std
                        + (1|region) + (0 + rest.age.std|hydrology), 
                        data = data_model_20y,
                        family = Gamma(link="log")
)
...
# the model we use is:
# target.hill.1 ~ rest.meth + land.use.hist + rest.age.std + (1 |region) + (1 |hydrology)
...
# (Intercept)             2.04489    0.24089   8.489   <2e-16 ***
#   rest.methmga            0.37462    0.20991   1.785   0.0743 .  
# rest.methres            0.40636    0.16936   2.399   0.0164 *  
#   rest.methdih            0.45868    0.18068   2.539   0.0111 *  
#   land.use.histgrassland  0.02324    0.11005   0.211   0.8327    
# rest.age.std           -0.02626    0.08618  -0.305   0.7606  

# --> restoration age is not significant

performance(restfact_targethill1_20y)
#' ICC = 0.422
#' The covariates explain 6 % of the variation in richness (R2 marg.)
#' The covariates and random effects explain 46 % of the variation. (R2 cond.)


### b report final model ----

# # Tidy up the model summary
# model_summary <- broom.mixed::tidy(restfact_targethill1_20y, effects = "fixed") %>%
...
#   write_csv(
#     here(
#       "outputs", "tables", "vegetation", "model_summary_restfact_targethill1_20y.csv"
#     )
#   )
...
save(restfact_targethill1_20y, data_model_targethill1_20y,
     file = here("outputs", "models", "vegetation",
                 "model_plants_restfact_targethill1_20y.Rdata"))
...
B1 <- glmmTMB(target.hill.1 ~ rest.meth * land.use.hist
              + (1|region) + (1|hydrology), 
              data = data_model,
              family = Gamma(link="log")
)
...
# 
# # use glmer.nb() for dredge
# B1a <- glmer.nb(target.hill.1 ~ rest.meth * land.use.hist 
#                 + (1|region) + (1|hydrology), 
#                 data = data_model)
# 
# full_model <- B1a
...
# top_model <- get.models(model_dredge, subset = 1)[[1]]
# summary(top_model)
# # target.hill.1 ~ land.use.hist + rest.meth + (1 | region) + (1 | hydrology)
# 
# options(na.action = "na.omit") # set back to default
...
# Estimate Std. Error z value Pr(>|z|)    
# (Intercept)             2.13958    0.21145  10.119  < 2e-16 ***
#   rest.methmga            0.30792    0.14574   2.113  0.03463 *  
#   rest.methres            0.31021    0.10574   2.934  0.00335 ** 
#   rest.methdih            0.35854    0.11364   3.155  0.00160 ** 
#   land.use.histgrassland  0.03178    0.08688   0.366  0.71453   

# --> rest.meth is significant

performance(restfact_targethill1)
#' ICC = 0.455
#' The covariates explain 7 % of the variation (R2 marg.)
...
# cus - mga -0.30792 0.1457 Inf  -2.113  0.1490
# cus - res -0.31021 0.1057 Inf  -2.934  0.0176
# cus - dih -0.35854 0.1136 Inf  -3.155  0.0087
# mga - res -0.00229 0.1282 Inf  -0.018  1.0000
# mga - dih -0.05062 0.1281 Inf  -0.395  0.9791
# res - dih -0.04833 0.0973 Inf  -0.497  0.9599
# 
# Results are averaged over the levels of: land.use.hist 
...
# # Tidy up the model summary
# model_summary <- broom.mixed::tidy(restfact_targethill1, effects = "fixed") %>%
#   select(term, estimate, std.error, statistic, p.value) %>%
#   mutate(across(c(estimate, std.error, statistic), round, 3)) %>%   # Rounding values
#   mutate(p.value = label_number(accuracy = 0.0001)(p.value)) %>% 
#   mutate(p.value = case_when(p.value < 0.001 ~ "< 0.001",
...
#   write_csv(
#     here(
#       "outputs", "tables", "vegetation", "model_summary_restfact_targethill1.csv"
#     )
#   )
...
save(restfact_targethill1, data_model_targethill1,
     file = here("outputs", "models", "vegetation",
                 "model_plants_restfact_targethill1.Rdata"))
...
glmm_rest_age <- glmer.nb(target.hill.1 ~ rest.age + (1|region) + (1|hydrology), data = data_model)
summary(glmm_rest_age)

glmm_rest_5 <- glmer.nb(target.hill.1 ~
                          rest.meth
                        * rest.age
                        + (1|region) + (1|hydrology)
                        , data = data_model
...
# Finale Grafik ---
ggplot(data_model, aes(rest.age, target.hill.1, color = region)) +
  geom_point() +
  theme_bw() +
...
B1_gamm <- gamm(
  target.hill.1 ~ rest.meth + land.use.hist + 
...
top_most_models
# second best model (by AICc) is target.hill.1 ~ land.use.hist + rest.meth + (1 | region) + (1 | hydrology)
# AIC 821.2433
...
B1a <- glm(target.hill.1 ~ rest.meth, data = data_model, family = Gamma(link = "log"))
...
glmm_dih_age <- glmer.nb(target.hill.1 ~ rest.age + (1|region) + (1|hydrology), data = data_dih)
glmm_mga_age <- glmer.nb(target.hill.1 ~ rest.age + (1|region), data = data_mga)
glmm_res_age <- glmer.nb(target.hill.1 ~ rest.age + (1|region), data = data_res)
glmm_cus_age <- glmer.nb(target.hill.1 ~ rest.age + (1|region), data = data_cus)
summary(glmm_dih_age)
summary(glmm_mga_age)
summary(glmm_res_age)
summary(glmm_cus_age)

plot(data_all$target.hill.1 ~ data_all$rest.age)

glmm_dry_age <- glmer.nb(target.hill.1 ~ rest.age + (1|region), data = data_dry)
glmm_fresh_age <- glmer.nb(target.hill.1 ~ rest.age + (1|region), data = data_fresh)
glmm_moist_age <- glmer.nb(target.hill.1 ~ rest.age + (1|region), data = data_moist)
