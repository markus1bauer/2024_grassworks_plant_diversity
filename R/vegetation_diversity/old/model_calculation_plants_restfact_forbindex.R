...
#   geom_quasirandom()
# (plot2 <- ggplot(data_all, aes(x = forb.index)) +
#   geom_histogram(binwidth = 1))
# plot3 <- ggplot(data_all, aes(x = forb.index)) +
#   geom_density()
# plot4 <- ggplot(data_all, aes(x = log(forb.index))) +
#   geom_density()
...
data_all %>% 
  select(id.site, forb.index, rest.meth, land.use.hist) %>% 
  group_by(rest.meth) %>% 
  identify_outliers(forb.index)
data_all %>% 
  select(id.site, forb.index, rest.meth, land.use.hist) %>% 
...
ggplot(data_all, aes(x = rest.meth, y = forb.index)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
  labs(title = "Restoration method")
ggplot(data_all, aes(x = land.use.hist, y = forb.index)) +
  geom_quasirandom(color = "grey") + geom_boxplot(fill = "transparent") +
  labs(title = "Previous landuse")
ggplot(data_all, aes(x = rest.age, y = forb.index)) +
...
coplot(forb.index ~ rest.age | region * hydrology,
       data = data_all,
...
coplot(forb.index ~ rest.age | rest.meth * land.use.hist,
       data = data_all,
       ylab = "Total species richness",
       xlab = "",
       panel = function(x, y, ...) {
...
ggplot(data_all, aes(rest.age, forb.index, color = rest.meth)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F) +
  facet_wrap(~land.use.hist)
ggplot(data_all, aes(rest.age, forb.index, color = hydrology)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F) +
  facet_wrap(~region)
...
ggplot(data_all, aes(x = interaction(land.use.hist, rest.meth), y = forb.index)) + geom_boxplot()
# interaction
...
interaction.plot(x.factor = data_all$rest.meth, trace.factor = data_all$region,
                 response = data_all$forb.index)
ggplot(data_all, aes(x = interaction(region, rest.meth), y = forb.index)) + geom_boxplot()
# interaction

interaction.plot(x.factor = data_all$rest.meth, trace.factor = data_all$hydrology,
                 response = data_all$forb.index)
ggplot(data_all, aes(x = interaction(rest.meth, hydrology), y = forb.index)) + geom_boxplot()
# interaction


## land.use.hist vs. X

ggplot(data_all, aes(rest.age, forb.index, color = land.use.hist)) +
  geom_point() + theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F)
# no clear interaction
...
interaction.plot(x.factor = data_all$land.use.hist, trace.factor = data_all$region,
                 response = data_all$forb.index)
ggplot(data_all, aes(x = interaction(region, land.use.hist), y = forb.index)) + geom_boxplot()
# no interaction

interaction.plot(x.factor = data_all$land.use.hist, trace.factor = data_all$hydrology,
                 response = data_all$forb.index)
ggplot(data_all, aes(x = interaction(hydrology, land.use.hist), y = forb.index)) + geom_boxplot()
# interaction
...
ggplot(data_all, aes(rest.age, forb.index, color = region)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "glm", method.args = list(family = poisson), se = F)
...
interaction.plot(x.factor = data_all$hydrology, trace.factor = data_all$region,
                 response = data_all$forb.index)
ggplot(data_all, aes(x = interaction(region, hydrology), y = forb.index)) + geom_boxplot()
# interaction
...
int_model <- glm(forb.index ~ rest.meth * land.use.hist, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction between rest.meth and land.use.hist

int_model <- glm(forb.index ~ rest.meth * rest.age, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction

int_model <- glm(forb.index ~ rest.meth * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> no interaction

int_model <- glm(forb.index ~ rest.meth * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# --> interaction
...
int_model <- glm(forb.index ~ land.use.hist * rest.age, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction 

int_model <- glm(forb.index ~ land.use.hist * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction

int_model <- glm(forb.index ~ land.use.hist * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# interaction
...
int_model <- glm(forb.index ~ rest.age * region, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# interaction between rest.age and region

int_model <- glm(forb.index ~ rest.age * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
...
int_model <- glm(forb.index ~ region * hydrology, data = data_all, family = Gamma(link = "log"))
check_overdispersion(int_model)
anova(int_model)
# no interaction
...
#' unclearity if there is a linear relationship between rest.age and forb.index
#' interaction between:
...
R7 <- glmmTMB(forb.index ~ 1 + (rest.age.std|region) + (1|hydrology), data = data_model, family = Gamma(link="log"))
R8 <- glmmTMB(forb.index ~ 1 + (1|region) + (rest.age.std|hydrology), data = data_model, family = Gamma(link="log"))
# -> Model convergence problem
R9 <- glmmTMB(forb.index ~ 1 + (rest.age.std|region) + (rest.age.std|hydrology), data = data_model, family = Gamma(link="log"))
# -> Model convergence problem
...
yB1 <- glmmTMB(forb.index ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
...
#' --> variance in residuals higher with smaller fitted values?
...
yB1_sl1 <- glmmTMB(forb.index ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (rest.age.std|region) + (1|hydrology), 
                   data = data_model_20y,
                   family = Gamma(link="log")
)
# --> Model convergence problem
yB1_sl2 <- glmmTMB(forb.index ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (1|region) + (rest.age.std|hydrology), 
                   data = data_model_20y,
                   family = Gamma(link="log")
)
yB1_sl3 <- glmmTMB(forb.index ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
                   + (0 + rest.age.std|region) + (1|hydrology), 
                   data = data_model_20y,
                   family = Gamma(link="log")
)
yB1_sl4 <- glmmTMB(forb.index ~ rest.meth * land.use.hist + rest.age.std * land.use.hist
...
# yB1a <- glmer.nb(forb.index ~ rest.meth * land.use.hist + rest.age.std * land.use.hist 
#                  + (1|region) + (1|hydrology), 
#                    data = data_model_20y)
# 
# full_model <- yB1a
...
# check for need of random slope
yB1_drop_sl1 <- glmmTMB(forb.index ~ rest.meth + land.use.hist + rest.age.std
                        + (rest.age.std|region) + (1|hydrology), 
                        data = data_model_20y,
                        family = Gamma(link="log")
)
# Model convergence problem
yB1_drop_sl2 <- glmmTMB(forb.index ~ rest.meth + land.use.hist + rest.age.std
                        + (1|region) + (rest.age.std|hydrology), 
                        data = data_model_20y,
                        family = Gamma(link="log")
)
yB1_drop_sl3 <- glmmTMB(forb.index ~ rest.meth + land.use.hist + rest.age.std
                        + (0 + rest.age.std|region) + (1|hydrology), 
                        data = data_model_20y,
                        family = Gamma(link="log")
)
yB1_drop_sl4 <- glmmTMB(forb.index ~ rest.meth + land.use.hist + rest.age.std
                        + (1|region) + (0 + rest.age.std|hydrology), 
                        data = data_model_20y,
                        family = Gamma(link="log")
)
...
# the model we use is:
# forb.index ~ rest.meth + land.use.hist + rest.age.std + (1 |region) + (1 |hydrology)

# load(file = here("outputs", "models", "vegetation","model_plants_restfact_hill0_yB1_final.Rdata"))


data_model_forbindex_20y <- data_all %>%
  filter(rest.age <= 20)

restfact_forbindex_20y <- glmmTMB(forb.index ~ rest.meth + land.use.hist + rest.age.std
                                + (1 |region) + (1 |hydrology),
                                data = data_model_forbindex_20y,
                                family = Gamma(link="log")
)
summary(restfact_forbindex_20y)
# Estimate Std. Error z value Pr(>|z|)   
# (Intercept)             0.51822    0.42865   1.209  0.22668   
# rest.methmga            0.40684    0.42246   0.963  0.33553   
# rest.methres            0.89268    0.33428   2.670  0.00758 **
#   rest.methdih            0.99886    0.35229   2.835  0.00458 **
#   land.use.histgrassland  0.06938    0.21886   0.317  0.75123   
# rest.age.std           -0.22203    0.17723  -1.253  0.21028 

# --> restoration age is not significant

performance(restfact_forbindex_20y)
#' ICC = 0.351
#' The covariates explain 14 % of the variation in richness (R2 marg.)
#' The covariates and random effects explain 44 % of the variation. (R2 cond.)


### b report final model ----

# Tidy up the model summary
model_summary <- broom.mixed::tidy(restfact_forbindex_20y, effects = "fixed") %>%
...
model_summary %>% 
  write_csv(
    here(
      "outputs", "tables", "vegetation", "model_summary_restfact_forbindex_20y.csv"
    )
  )
...
save(restfact_forbindex_20y, data_model_forbindex_20y,
     file = here("outputs", "models", "vegetation",
                 "model_plants_restfact_forbindex_20y.Rdata"))
...
B1 <- glmmTMB(forb.index ~ rest.meth * land.use.hist
              + (1|region) + (1|hydrology), 
              data = data_model,
              family = Gamma(link="log")
)
...
#' Is there a pattern in here?
#' negative fitted values?
#' maybe higher variance in residuals with low values?
...
# 
# # use glmer.nb() for dredge
# B1a <- glmer.nb(forb.index ~ rest.meth * land.use.hist 
#                 + (1|region) + (1|hydrology), 
#                 data = data_model)
# 
# full_model <- B1a
...
# top_model <- get.models(model_dredge, subset = 1)[[1]]
# summary(top_model)
# # forb.index ~ land.use.hist + rest.meth + (1 | region) + (1 | hydrology)
# 
# options(na.action = "na.omit") # set back to default
...
# the model we use is:
# forb.index ~ rest.meth + land.use.hist + (1 |region) + (1 |hydrology)
...
restfact_forbindex <- glmmTMB(forb.index ~ rest.meth + land.use.hist
                            + (1 |region) + (1 |hydrology),
                            data = data_model_forbindex,
                            family = Gamma(link="log")
)
summary(restfact_forbindex)
# Estimate Std. Error z value Pr(>|z|)    
# (Intercept)             1.08086    0.30898   3.498 0.000469 ***
#   rest.methmga           -0.05678    0.30129  -0.188 0.850528    
# rest.methres            0.43372    0.22656   1.914 0.055571 .  
# rest.methdih            0.48907    0.22796   2.145 0.031918 *  
#   land.use.histgrassland  0.17008    0.17287   0.984 0.325180 
...
emm.rest.meth <- emmeans(restfact_forbindex, ~ rest.meth)
emm.rest.meth
...
# cus - mga   0.0568 0.301 Inf   0.188  0.9976
# cus - res  -0.4337 0.227 Inf  -1.914  0.2219
# cus - dih  -0.4891 0.228 Inf  -2.145  0.1389
# mga - res  -0.4905 0.263 Inf  -1.866  0.2426
# mga - dih  -0.5458 0.253 Inf  -2.158  0.1351
# res - dih  -0.0553 0.202 Inf  -0.274  0.9928
# 
...
# Tidy up the model summary
model_summary <- broom.mixed::tidy(restfact_forbindex, effects = "fixed") %>%
  select(term, estimate, std.error, statistic, p.value) %>%
  mutate(across(c(estimate, std.error, statistic), round, 3)) %>%   # Rounding values
  mutate(p.value = label_number(accuracy = 0.0001)(p.value)) %>% 
  mutate(p.value = case_when(p.value < 0.001 ~ "< 0.001",
...
model_summary %>% 
  write_csv(
    here(
      "outputs", "tables", "vegetation", "model_summary_restfact_forbindex.csv"
    )
  )
...
save(restfact_forbindex, data_model_forbindex,
     file = here("outputs", "models", "vegetation",
                 "model_plants_restfact_forbindex.Rdata"))
...
glmm_rest_age <- glmer.nb(forb.index ~ rest.age + (1|region) + (1|hydrology), data = data_model)
summary(glmm_rest_age)

glmm_rest_5 <- glmer.nb(forb.index ~
                          rest.meth
                        * rest.age
                        + (1|region) + (1|hydrology)
                        , data = data_model
...
# Finale Grafik ---
ggplot(data_model, aes(rest.age, forb.index, color = region)) +
  geom_point() +
  theme_bw() +
...
B1_gamm <- gamm(
  forb.index ~ rest.meth + land.use.hist + 
...
top_most_models
# second best model (by AICc) is forb.index ~ land.use.hist + rest.meth + (1 | region) + (1 | hydrology)
# AIC 821.2433
...
B1a <- glm(forb.index ~ rest.meth, data = data_model, family = "poisson")
...
glmm_dih_age <- glmer.nb(forb.index ~ rest.age + (1|region) + (1|hydrology), data = data_dih)
glmm_mga_age <- glmer.nb(forb.index ~ rest.age + (1|region), data = data_mga)
glmm_res_age <- glmer.nb(forb.index ~ rest.age + (1|region), data = data_res)
glmm_cus_age <- glmer.nb(forb.index ~ rest.age + (1|region), data = data_cus)
summary(glmm_dih_age)
summary(glmm_mga_age)
summary(glmm_res_age)
summary(glmm_cus_age)

plot(data_all$forb.index ~ data_all$rest.age)

glmm_dry_age <- glmer.nb(forb.index ~ rest.age + (1|region), data = data_dry)
glmm_fresh_age <- glmer.nb(forb.index ~ rest.age + (1|region), data = data_fresh)
glmm_moist_age <- glmer.nb(forb.index ~ rest.age + (1|region), data = data_moist)
